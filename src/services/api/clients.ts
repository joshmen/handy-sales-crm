...
class ClientService {
  ...
  async createClient(clientData: CreateClientRequest): Promise<Client> {
    try {
      const response = await api.post<ApiResponse<Client>>(this.basePath, clientData);
      return handleApiResponse(response);
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  async updateClient(clientData: UpdateClientRequest): Promise<Client> {
    try {
      ...
      const response = await api.put<ApiResponse<Client>>(`${this.basePath}/${id}`, updateData);
      return handleApiResponse(response);
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  async toggleClientStatus(id: string): Promise<Client> {
    try {
      const response = await api.patch<ApiResponse<Client>>(`${this.basePath}/${id}/toggle-status`);
      return handleApiResponse(response);
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  async getClientsByZone(zone: string): Promise<Client[]> {
    try {
      const response = await api.get<ApiResponse<Client[]>>(`${this.basePath}/by-zone/${zone}`);
      return handleApiResponse(response);
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  async getClientsByType(type: string): Promise<Client[]> {
    try {
      const response = await api.get<ApiResponse<Client[]>>(`${this.basePath}/by-type/${type}`);
      return handleApiResponse(response);
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  async exportClients(format: 'csv' | 'excel' = 'csv'): Promise<Blob> {
    try {
      const response = await api.get<Blob>(`${this.basePath}/export`, {
        params: { format },
        responseType: 'blob' as const, // asegura la inferencia correcta
      });
      return response.data; // <- ahora es Blob
    } catch (error) {
      throw handleApiError(error);
    }
  }
