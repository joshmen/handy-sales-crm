...
///  - VENDEDOR sólo ve sus propios cobros (CobroService.ObtenerCobrosAsync inyecta usuarioId del JWT).
...
///  - Monto > 0 (BR-C-monto).
///  - Fecha de cobro no futura ni > 20 años antigua (BR-050d).
///  - IDOR: ObtenerPorId / EstadoCuenta scoped por tenant.
///
/// Patrón: igual a MobileClienteEndpointsTests — Mock&lt;ICurrentTenant&gt; +
...
#pragma warning restore CS0618
    ...
    // ─────────────────────────────────────────────────────────────
    // GET /api/mobile/cobros/mis-cobros — RBAC scoping
    // ─────────────────────────────────────────────────────────────

    [Fact]
    public async Task ObtenerCobros_AsVendedor_InjectsOwnUsuarioIdFilter()
    {
        // Arrange — VENDEDOR no debe poder listar cobros de otros vendedores.
        ...
        // Assert — el repo recibió VendedorId aunque el caller pasó usuarioId=null.
    ...
    {
        // Arrange — VENDEDOR intenta forzar usuarioId=otroVendedor; el Service debe sobrescribir.
        var tenant = BuildVendedorTenant(userId: VendedorId);
        _cobroRepoMock
            .Setup(r => r.ObtenerCobrosAsync(TenantId, null, null, null, VendedorId))
            .ReturnsAsync(new List<CobroDto>());
        ...
        // Act — caller pasa OtherVendedorId; el service debería igualmente forzar VendedorId.
    ...
    {
        // Arrange — ADMIN ve todos los cobros del tenant.
    ...
    {
        // ADMIN sí puede filtrar manualmente por un vendedor específico (dashboard).
        var tenant = BuildAdminTenant();
        _cobroRepoMock
            .Setup(r => r.ObtenerCobrosAsync(TenantId, null, null, null, VendedorId))
            .ReturnsAsync(new List<CobroDto>());
    ...
    // ─────────────────────────────────────────────────────────────
    // POST /api/mobile/cobros — Validaciones de negocio + cross-tenant
    // ─────────────────────────────────────────────────────────────
    ...
    {
        // Arrange — cliente NO pertenece al tenant del caller (repo retorna null).
        var tenant = BuildVendedorTenant();
        ...
        // Assert — debe lanzar antes de tocar el CobroRepository.
        await act.Should().ThrowAsync<InvalidOperationException>()
    ...
    public async Task CrearCobro_ThrowsWhenFechaCobroIsFuture()
    {
        ...
        var dto = new CobroCreateDto(
            ...
            FechaCobro: DateTime.UtcNow.AddDays(7), Referencia: null, Notas: null);
    ...
    public async Task CrearCobro_ThrowsWhenFechaCobroIsTooOld()
    {
        ...
        var dto = new CobroCreateDto(
            PedidoId: null, ClienteId: 10, Monto: 100m, MetodoPago: 0,
            FechaCobro: DateTime.UtcNow.AddYears(-25), Referencia: null, Notas: null);
    ...
    {
        // Arrange — vendedor crea cobro válido con pedido confirmado.
        var tenant = BuildVendedorTenant(userId: VendedorId);
        _clienteRepoMock.Setup(r => r.ObtenerPorIdAsync(10, TenantId)).ReturnsAsync(BuildCliente(10));
        ...
        // Assert — repo recibió el tenantId y el userId del caller, no del DTO.
        newId.Should().Be(555);
        _cobroRepoMock.Verify(
    ...
    // ─────────────────────────────────────────────────────────────
    // GET /api/mobile/cobros/saldos — Tenant scoping
    // ─────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────
    // GET /api/mobile/cobros/cliente/{id}/estado-cuenta — IDOR / tenant scope
    // ─────────────────────────────────────────────────────────────
    ...
    public async Task ObtenerEstadoCuenta_ReturnsNull_WhenClienteIsInDifferentTenant()
    {
        ...
        // → el endpoint responde 404. Esto previene IDOR cross-tenant.
        var tenant = BuildVendedorTenant(tenantId: TenantId);
    ...
    // ─────────────────────────────────────────────────────────────
    // GET /api/mobile/cobros/{id} — equivalente a ObtenerPorIdAsync (defensa IDOR)
    // ─────────────────────────────────────────────────────────────
