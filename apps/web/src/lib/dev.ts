...
export const generateMockClient = (overrides: Record<string, unknown> = {}) => {
  ...
  return {
    id: generateMockId(),
    code: `CLI-${Math.floor(Math.random() * 9999)
      .toString()
      .padStart(4, '0')}`,
    name: names[Math.floor(Math.random() * names.length)],
    email: `cliente${Math.floor(Math.random() * 999)}@email.com`,
    phone: `+52 644 ${Math.floor(Math.random() * 999)
      .toString()
    ...
    state: 'Sonora',
    zipCode: `83${Math.floor(Math.random() * 999)
      .toString()
      .padStart(3, '0')}`,
...
export const generateMockProduct = (overrides: Record<string, unknown> = {}) => {
  ...
  return {
    id: generateMockId(),
    code: `PROD-${Math.floor(Math.random() * 9999)
      .toString()
      .padStart(4, '0')}`,
    name: names[Math.floor(Math.random() * names.length)],
...
export const DevInfo: React.FC = () => {
  ...
  return React.createElement(
    ...
    {
      className:
        'fixed bottom-4 right-4 bg-black text-white text-xs p-2 rounded opacity-50 pointer-events-none z-50',
    },
    React.createElement('div', null, `ENV: ${process.env.NODE_ENV}`),
    React.createElement('div', null, `API: ${process.env.NEXT_PUBLIC_API_URL}`)
  );
};
