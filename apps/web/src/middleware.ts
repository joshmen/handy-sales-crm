...
// Auth.js v5: el wrapper `auth()` corre la función en cada ruta del matcher y
// expone `req.auth` = la SESIÓN (resultado del callback session), no el token JWT.
// Los campos del token v4 (token.role / token.isImpersonating / token.id) se leen
// ahora de la sesión: session.user.role / session.isImpersonating / session.user.id.
//
// CLAVE Edge: el middleware crea su propia instancia NextAuth desde authConfig
// (Edge-safe, sin axios). NO importa `auth` de @/lib/auth, que arrastra axios via
// los providers/jwt y rompe el Edge Runtime (setImmediate no existe en Edge).
...
export default auth((req) => {
  ...
  // Páginas públicas (incluye /billing/suspended, que antes vivía en el callback
  // `authorized` del withAuth v4 — ahora se maneja aquí en el body).
  const isPublicPage =
    pathname === '/' ||
  ...
  // isImpersonating se almacena en el JWT firmado (tamper-proof) y se expone en la sesión
  if (userRole === UserRole.SUPER_ADMIN && !session?.isImpersonating) {
  ...
  const effectiveRole = (userRole === UserRole.SUPER_ADMIN && session?.isImpersonating)
    ? UserRole.ADMIN
    : userRole;
