    ...
    if (step === 'result') {
      return (
        <div className="flex items-center justify-end gap-3">
          <button
            onClick={handleReset}
            className="px-4 py-2 text-sm font-medium text-foreground/80 bg-white border border-border-default rounded-md hover:bg-surface-1"
          >
            {t('importAnother')}
          </button>
          <button
        ...
        {step === 'upload' && (
          <>
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1">{t('csvOnly')}</p>
              <input
                ref={fileInputRef}
                type="file"
                accept=".csv"
        ...
        {step === 'preview' && file && (
          <>
            ...
            <div className="flex items-center gap-3 p-3 bg-surface-1 rounded-lg">
              ...
              <button onClick={handleBackToUpload} className="text-muted-foreground hover:text-foreground/70" title={t('changeFile')}>
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <>
            <div className="overflow-x-auto border border-border-subtle rounded-lg max-h-[400px] overflow-y-auto">
              <table className="w-full text-xs">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-surface-3">
                    <th className="px-2 py-2 text-left w-8 bg-surface-3">
                      <button
                        onClick={toggleAllVisible}
                        className={`w-4 h-4 rounded flex items-center justify-center border transition-colors ${
                          allVisibleSelected ? 'bg-green-600 border-green-600' :
                    ...
                    <th className="px-2 py-2 text-left text-muted-foreground font-normal bg-surface-3 w-8">#</th>
                    {headers.map((h, i) => (
                      <th key={i} className="px-2 py-2 text-left font-medium text-foreground/80 bg-surface-3 whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  </tr>
                      ...
                      >
                        ...
                        {row.map((cell, j) => (
                          <td
                            ...
                            title={cell}
                          >
                            {cell || <span className="text-muted-foreground/60 italic">{t('empty')}</span>}
                          </td>
                        ))}
                      </tr>
                    );
              ...
              <div className="flex items-center justify-between px-1">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="px-2 py-1 text-[11px] text-foreground/70 border border-border-subtle rounded hover:bg-surface-1 disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    «
                  </button>
                  <button
                  ...
                  <span className="px-2 text-[11px] text-foreground/80 font-medium">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className={`p-3 rounded-lg text-center ${result.errores > 0 ? 'bg-red-50' : 'bg-surface-1'}`}>
                ...
                <p className={`text-xs ${result.errores > 0 ? 'text-red-600' : 'text-muted-foreground'}`}>{t('withErrors')}</p>
              </div>
            </div>
