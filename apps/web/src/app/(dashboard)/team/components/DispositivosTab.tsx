...
export function DispositivosTab() {
  ...
  const executeRevoke = useCallback(async () => {
    ...
    try {
      ...
      toast.success(`Sesion de ${session.usuarioNombre} revocada exitosamente`);
      await fetchSessions();
    } catch (err) {
      console.error('Error al revocar sesion:', err);
      toast.error('Error al revocar la sesion. Intenta de nuevo.');
    } finally {
      setRevokingId(null);
    }
  ...
  const renderSkeleton = () => (
    <div className="space-y-0">
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          key={i}
          className="flex items-center gap-4 px-4 py-3.5 border-b border-border animate-pulse"
        >
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-end gap-2">
        <button
          ...
          className="flex items-center gap-1.5 px-3 sm:px-4 py-2 text-xs font-medium text-foreground border border-border rounded-md hover:bg-muted transition-colors disabled:opacity-50"
        >
          {cleaningExpired ? (
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        >
          ...
          <option value="1">Sesion cerrada</option>
        ...
        <div className="hidden sm:block ml-auto text-xs text-gray-500">
          {!loading && `${totalItems} sesion${totalItems !== 1 ? 'es' : ''}`}
        </div>
      </div>
              ...
              <>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th scope="col" className="px-4 py-2.5 text-left text-xs font-semibold text-foreground w-[180px]">
                        Vendedor
                      </th>
                      <th scope="col" className="px-4 py-2.5 text-left text-xs font-semibold text-foreground w-[220px]">
                        Dispositivo
                      </th>
                      <th scope="col" className="px-4 py-2.5 text-left text-xs font-semibold text-foreground w-[130px]">
                        Conectado desde
                      </th>
                      <th scope="col" className="px-4 py-2.5 text-left text-xs font-semibold text-foreground w-[130px]">
                        Ultima Actividad
                      </th>
                      <th scope="col" className="px-4 py-2.5 text-left text-xs font-semibold text-foreground w-[110px]">
                        Estado
                      </th>
                      <th scope="col" className="px-4 py-2.5 text-right text-xs font-semibold text-foreground w-[100px]">
                        Acciones
                      </th>
                    </tr>
                  </thead>
      ...
      <Modal
        ...
        title={confirmAction?.type === 'revoke' ? 'Revocar sesion' : 'Limpiar sesiones expiradas'}
        size="sm"
      >
        {confirmAction?.type === 'revoke' && confirmAction.session && (
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              ¿Revocar la sesion de <span className="font-semibold text-gray-900">{confirmAction.session.usuarioNombre}</span> en{' '}
            ...
            <div className="bg-red-50 border border-red-200 rounded-md px-3 py-2">
              <p className="text-xs text-red-700">El usuario sera desconectado inmediatamente.</p>
            </div>
        ...
        {confirmAction?.type === 'clean' && (
          <div className="space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-md px-3 py-2">
              <p className="text-xs text-amber-700">Esto eliminara sesiones inactivas por mas de 30 dias.</p>
            </div>
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setConfirmAction(null)}
