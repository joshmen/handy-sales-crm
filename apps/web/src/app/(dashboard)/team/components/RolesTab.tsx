...
export function RolesTab() {
  ...
  // Hook separado para obtener todos los roles para estadisticas
  const {
    data: allRolesData,

    execute: loadStats,
  } = useRoles(); // Sin parametros de paginacion para obtener todos
  ...
  // Obtener todos los roles para estadisticas
  const allRoles = Array.isArray(allRolesData) ? allRolesData : [];

  // Estadisticas basadas en todos los roles del backend
  const stats = {
  ...
  const importColumns: ImportColumn[] = [
    ...
    },
    {
      ...
      transformer: (value: unknown) => {
        ...
        return ['true', '1', 'yes', 'si'].includes(val) ? 'true' : 'false';
      },
    },
  ];
  ...
  const handleImportRoles = async (roles: Partial<Role>[], errors: unknown[]) => {
    try {
      ...
      if (errorCount > 0) {
        toast({
          title: 'Algunos errores en importacion',
    ...
    <div className="space-y-6">
        ...
        >
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="create-descripcion">Descripcion</Label>
              <Input
          ...
          <DialogContent>
            ...
            <div className="py-4">
              <p className="text-muted-foreground">
                ¿Estas seguro de que deseas eliminar el rol <strong>{selectedRole?.nombre}</strong>?
                Esta accion no se puede deshacer y puede afectar a usuarios que tengan este rol
                asignado.
              </p>
            </div>
            <DialogFooter>
